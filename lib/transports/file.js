...
file.prototype.setupGet = function(offset) {
  ...
  if (self.file === '$') {
    ...
    if (path.extname(self.file) === '.gz'){
      self.metaStream = fs.createReadStream(self.file).pipe(zlib.createGunzip());
    }
    else {
  ...
  if (!self.stream) {
    if (self.file === '$') {
      ...
      if (fs.existsSync(self.file)) {
        ...
        if (path.extname(self.file) === '.gz'){
          self.stream = zlib.createGzip();
          self.stream.pipe(fs.createWriteStream(self.file));
        }
        else {
